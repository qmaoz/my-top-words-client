...
export default function WordSetPage() {
  ...
  let numWordsLearned = 0;
  activeItem?.words?.map((obj) => {
    if (obj?.isWordLearned) numWordsLearned++;
  });
  ...
  const handleToggleSave = async () => {
    const data = await dispatch(toggleWordSetSave({ id: wordSetId }));
    if (!data.payload) {
      return alert('Не вдалося змінити статус набору. Ймовірно, сталася помилка');
    }
  };

  const handleToggleIsPublic = async () => {
    if (isOwnWordSet) {
      if (window.confirm('Ви дійсно хочете змінити публічний статус свого набору?')) {
  ...
  const handleDeleteWordSet = async () => {
    if (isOwnWordSet) {
      if (window.confirm('Ви дійсно хочете видалити свій набір?')) {
        const data = await dispatch(deleteWordSet(wordSetId));
        if (!data.payload) {
          return alert('Не вдалося видалити набір. Ймовірно, сталася помилка');
        }
        navigateBack();
      }
    }
  ...
  const onWordSetNameChange = async (values) => {
    if (isOwnWordSet) {
      if (values.word_set_name.trim() != activeItem.name) {
        ...
        if (!data.payload) {
          return alert('Не вдалося оновити набір. Ймовірно, сталася помилка');
        }
        reset();
      }
  ...
  return (
    <>
      <div className="word-set-page container">
        <div className="word-set-page-header">
          <form onSubmit={handleSubmit(onWordSetNameChange)}>
            {!isEditing ? <>
            ...
            <div className="word-set-page-header__right">
              {activeItem && isAuth && <>
                {!isEditing &&
              ...
              {!isEditing && activeItemStatus === 'loaded' && totalWords > 0 && <>
                <Box sx={{ boxShadow: 2 }} className='mb-3 rounded'>
                  <Link to={`/translate-exercise/${wordSetId}`} className="exercise-card rounded">
                    ...
                    <h4>Потренуватися в тренажері на переклад</h4>
                  </Link>
                </Box>
              </>}
